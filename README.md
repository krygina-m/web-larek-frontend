...
### 2. Класс `ContactsOrder` (Котакты заказчика)
...
### 3. Класс `OrderAddress` (Адрес заказа)
Класс для хранения информации о адресе заказчика и способе оплаты.
...
- **items: string[]** - массив купленных товаров
...
Отвечает за отображение данных о заказе. Содержит сеттеры для работы с полями.

В конструктор принимает HTML-элемент формы и экземпляр EventEmitter для инициализации событий.

#### Поля:
  - **payment: PaymentMethod** - способ оплаты
	- **email: string** - e-mail заказчика
	- **phone: string** - телефон заказчика
	- **address: string** - адрес заказчика 
...
- `OrderAddress.address:change`- изменение поля ввода адреса доставки

- `OrderAddress.payment:change` - изменение поля ввода способа оплаты

- `OrderAddress:submit` - сохранение данных в форме ввода способа оплаты и адреса доставки
...
- `ContactsOrder:submit` - сохранение данных в форме ввода контактных данных покупателя
